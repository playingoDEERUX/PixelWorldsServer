...
namespace PixelWorldsServer.WorldSystem
{
    public interface WorldInterface
    {
        ...
        {
            public short foregroundID, backgroundID, liquidID, gateID, 
				foregroundHits, backgroundHits, liquidHits, gateHits, 
				foregroundBuffers, backgroundBuffers, liquidBuffers, gateBuffers;
            WorldLayer[] layers;

			public long[] lastHit;
        }

		public struct LockPoint // more efficient point structure to decrease ram usage
        {
			short x, y;
			public LockPoint(short x, short y)
			{
				this.x = x;
				this.y = y;
			}
		...
		public struct Lock
        {
			...
			public int ownerID;
			public byte type;
			public int[] accessList; // max 20
			public LockPoint[] claimedArea;

			public Lock(short x, short y, int aSize, int oID, byte t, int maxAcc = 20)
            {
				posX = x; posY = y;
				claimedArea = new LockPoint[aSize];
				ownerID = oID;
				type = t;
				accessList = new int[maxAcc]; // array of userID's
            }
        }
